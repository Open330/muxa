...
실환경 검증 뒤 최신 main을 통합했다. main의 릴리스 버전 0.8.54를 유지하고,
로컬 검증용 0.8.51+quiet.1 버전 변경은 PR 차이에서 제거했다. 위 측정 자료는
`5849163` 빌드의 실환경 기록이며 최신 main 빌드의 실환경 측정으로 해석하지 않는다.
실행 중인 로컬 검증 바이너리는 이 PR 통합 과정에서 다시 교체하지 않았다.

자체 리뷰에서 조용한 메시지의 보존·조회, 명시적 알림 및 기존 전역 wake 규칙,
독립적인 완료 응답 전달, 이미 준비된 직접 전달의 복구, 과거 저장 레코드 호환,
MCP/CLI boolean 파싱과 안내 문구를 확인했다. 일반 notice의 기본값 변경은
운영자 화면에서 보내는 one-way notice에도 적용된다. 즉시 전달할 인계는
명시적 `notify=true`를 지원하는 CLI/MCP를 사용한다.
